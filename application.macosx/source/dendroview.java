...
public class dendroview extends PApplet {
...
public void draw() { 
  background(0.2f); //make background grey

  pushMatrix(); //save origintransform

  translate(width/2, height/2, -30); 

  newXmag = (mouseX-zeroX)/PApplet.parseFloat(width) * TWO_PI;
  newYmag = (mouseY-zeroY)/PApplet.parseFloat(height) * TWO_PI;
...
public void paintInstance(int x, float y, float z) {
  if (flat) {z=flatZ;}
...
public void paintLine(Instance a, Instance b, float weight) {
  pushMatrix();
  stroke(0.5f, weight/10);
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#DFDFDF", "dendroview" });
  }
}
